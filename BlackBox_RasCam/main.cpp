...
int main(int argc, char *argv[])
{
	...
	const char* record_list_path= "/home/pi/openCVtest/BlackBox_RasCam/Record/Dir_List.txt";
	FILE *fwp, *frp;
	int Dir_length;
	int Dir_Count=0;
	// Dir_Count의 값을 백업하고 불러오기 위한 파일포인터및 경로
	const char* listcount_path= "/home/pi/openCVtest/BlackBox_RasCam/Record/Dir_Count.txt";
	FILE *fwp2, *frp2;
...
//	cout << "Video Size : " << out_size << endl;
	...
	//
		...
		err_Check = strftime(path_buffer,sizeof(path_buffer), "/home/pi/openCVtest/BlackBox_RasCam/Record/%Y%m%d_%H/",ptm);
		if(err_Check ==0 )	
		{
...
//		puts(time_buffer);
	...
	//
		...
		// 인자로 주어진 파일/디렉토리의 마운트된 곳의 정보를 가져온다.
	    	if ( statfs("/home/pi/openCVtest/BlackBox_RasCam/Record/", &s) !=0) 
		{
		 	perror("statfs");
		  	return -6;
	      	}
	...
	// 파일 시스템의 남은 용량이 25% 이하이면 가장 오래된 Directory를 삭제한다.
	//
		if(free_percent<=25)
	...
	//
		...
		//
		VideoWriter outputVideo;
		outputVideo.open(buffer,VideoWriter::fourcc('X','V','I','D'), fps, frameSize, true);
		if(!outputVideo.isOpened())
		{
			puts("(EEROR) 영상 저장을 위한 초기화 작업중 에러 발생");
			return -3;
		}
	...
	//
		...
		//
			...
			double myFontSize = 2;		// 글자크기 설정
			Scalar Red(0,0,255);		// 색상설정
			int myFontThickness = 3;	// 굵기설정
			putText(image, currenttime, Point(10,950), myFont,myFontSize, Red, myFontThickness); 
			
	
	
		// 화면에 영상을 보여줌
		//
			imshow("BlackBox_RasCam",image);
		...
		//
			if(waitKey(20) == 27) {CONTINUED = 0; break;}
